...
export default function AdminAcademicPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'faculty_administrators'), (snap) => {
      setStaff(
        snap.docs.map((d) => ({
          id: d.id,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateDoc(doc(db, 'faculty_administrators', editingUser.id), {
  ...
  const handleToggleStatus = async (user: StaffUser) => {
    ...
    try {
      await updateDoc(doc(db, 'faculty_administrators', user.id), { status: newStatus });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Faculty Administrator Management</h1>
          <p className="text-muted-foreground">Manage Faculty Administrator accounts</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-l-4 border-l-indigo-500">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Total Faculty Administrators</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No Faculty Administrator accounts found</p>
            </div>
          ) : (
